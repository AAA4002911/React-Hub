...
const ThemeSwitcher = () => {
    ...
    return (
        <aside className={styles.wrapper} style={{backgroundColor: isColorPicking ? 'hsl(var(--muted) / .6)' : "transparent"}}>
            {isColorPicking ? (
                <>
                    <button className={`btn ${styles.close}`} aria-label="color picking close" onClick={() => setIsColorPicking(false)}>❌</button>
                ...
                <div className={styles.btns}>
                    <button className="btn" onClick={handleTheme} aria-label={`Change theme to ${theme === 'light' ? 'dark' : 'light'} Mode`}> {theme === 'light' ? <>🌜</> : <>☀️</>}</button>
                    <button className="btn" onClick={() => setIsColorPicking(true)} aria-label="Enable color picking mode">🛠️</button>
                </div>
            )}
        </aside>
    )
